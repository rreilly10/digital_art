...
class SpliceHorizontallyTransform(ImageTransform):
    def apply(self, image, target_image, step_size: int = 1) -> np.array:
        ...
        switch = True
        for i in range(0, len(image), step_size):
            if switch:
...
if __name__ == "__main__":
    splice_horizontally = SpliceHorizontallyTransform()
    ...
    out = splice_horizontally.apply(image=image, target_image=image2, step_size=20)
